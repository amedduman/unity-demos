...
namespace XNode
{
    ...
    public abstract class Node : ScriptableObject
    {
        ...
        /// </summary>
        [SerializeField] NodePortDictionary ports = new();
        ...
        /// added by Amed
        /// </summary>
        /// <param name="exitPortName"></param>
        protected void ExecuteNextNode(string exitPortName)
        {
            foreach (var nodePort in Ports)
            {
                if (nodePort.fieldName == exitPortName) // If field name is Exit
                {
                    if (nodePort.ConnectionCount == 0) break;
                    if (nodePort.IsConnected)
                    {
                        var n =nodePort.Connection.node as IExecutableNode;
                        n.Execute();
                    }
                }
            }
        ...
        [Serializable]
        class NodePortDictionary : Dictionary<string, NodePort>, ISerializationCallbackReceiver
        {
            [SerializeField] List<string> keys = new();
            [SerializeField] List<NodePort> values = new();
        ...
        NodePort AddInstancePort(Type type, NodePort.IO direction,
        ...
        NodePort AddDynamicPort(Type type, NodePort.IO direction,
            ConnectionType connectionType = ConnectionType.Multiple,
            TypeConstraint typeConstraint = TypeConstraint.None, string fieldName = null)
        {
            if (fieldName == null)
